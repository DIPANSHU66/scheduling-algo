...
const FCFS = ({ processes }) => {
  ...
  const addNewProcesses = (newProcesses) => {
    const filteredNewProcesses = newProcesses.filter(
      (newProcess) =>
    ...
    const simulationStep = async () => {
      ...
      if (simulationState.executingProcess) {
        ...
        const continueExecution = async () => {
          for (
            let t = currentTime + 1;
            t <= simulationState.processEndTime;
            t++
          ) {
            if (!isSimulating || isPaused) break;
            setCurrentTime(t);
            await new Promise((resolve) =>
              setTimeout(resolve, getAnimationDelay(100))
            );
          }
          setAnimatingTimeJump(false);
        };
        ...
        if (isSimulating && !isPaused) {
          ...
          setPendingProcesses((prev) =>
            prev.filter((p) => p.name !== process.name)
          );
          setCurrentProcess(null);
          setFadeOutProcess(null);
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, getAnimationDelay(500))
      );
      ...
      for (let i = 1; i < arrivedProcesses.length; i++) {
        ...
        if (
          parseInt(processToCompare.arrivalTime) <
          parseInt(minProcess.arrivalTime)
        ) {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, getAnimationDelay(300))
          );
        }
      }
      ...
      setPendingProcesses((prev) =>
        prev.filter((p) => p.name !== nextProcess.name)
      );
      setCurrentProcess(null);
      setFadeOutProcess(null);
      setSimulationState((prev) => ({
        ...
        <div className="inline-block px-4 py-2 bg-black text-white rounded-lg border border-white overflow-hidden relative">
          ...
          <span
            className={`inline-block min-w-[3ch] text-center ${
              animatingTimeJump ? "animate-pulse text-yellow-400" : ""
            }`}
          >
            {currentTime}
          </span>
            ...
            {pendingProcesses.map((p) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-1 mt-2 text-sm">
                  <p className="bg-gray-900 rounded p-1">
      ...
      <div className="w-full bg-black p-4 rounded-lg border border-white">
        ...
        {ganttChart.length > 0 ? (
          <div className="relative">
            ...
            <div className="flex h-20 mb-12 relative">
              {ganttChart.map((p, index) => {
                ...
                const prevEndTime =
                  index > 0 ? ganttChart[index - 1].endTime : 0;
                ...
                const processWidth =
                  ((p.endTime - p.startTime) / totalTime) * 100;
          ...
          <p className="text-gray-400 italic text-center py-8">
            Gantt chart will appear here after simulation starts
          </p>
        )}
      </div>
              ...
              <tbody>
                {ganttChart.map((p) => {
                  ...
                  return (
                    <tr key={p.name}>
                      <td
                        className="py-2 px-4 border border-white font-medium"
                        style={{ color: processColors[p.name] || "#3498db" }}
                      >
                        {p.name}
                      </td>
                ...
                {ganttChart.length > 0 && (
                  <tr className="bg-gray-900 font-semibold">
                    <td
                      className="py-2 px-4 border border-white text-right"
                      colSpan="5"
                    >
                      Average
                    </td>
                    <td className="py-2 px-4 border border-white text-center">
                      {(
                        ganttChart.reduce(
                          (sum, p) =>
                            sum + (p.endTime - parseInt(p.arrivalTime)),
                          0
                        ) / ganttChart.length
                      ).toFixed(2)}
                    </td>
                    <td className="py-2 px-4 border border-white text-center">
                      {(
                        ganttChart.reduce(
                          (sum, p) =>
                            sum + (p.startTime - parseInt(p.arrivalTime)),
                          0
                        ) / ganttChart.length
                      ).toFixed(2)}
                    </td>
